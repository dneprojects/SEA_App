...
- **Dokumentation: geschaltete und geregelte Verbraucher** — neues Kapitel zur Unterscheidung (Waschmaschine/Waeschetrockner geschaltet, Wallbox/Batterie geregelt) samt Startprinzip grosser geschalteter Verbraucher: Einschalten erst, wenn PV-Ueberschuss plus Batterie-Unterstuetzung die volle Leistung decken, Budgetvergabe nach Prioritaet, Notstrom-Reserve als Untergrenze.
- **Neu: eingebettete Dokumentation** — eine verstaendliche Anleitung direkt in der App (Seitenleiste -> Dokumentation, neben dem Menue eingebettet statt in neuem Tab) mit Inhaltsverzeichnis, Querverweisen und Diagrammen: was SEA macht, Einrichtung Schritt fuer Schritt, Planung/Prognose/Simulation/Analysen und die Bedienung der schaltbaren Maschinen. Sachlicher Stil im Layout der Habitron-Dokumentation, mit wenig Fachbegriffen; ohne redundante Kopfzeile (App-Header + Titel genuegen).
- **Rand-Navigation auch auf der Doku-Seite** — „ueber den Rand scrollen wechselt die Seite" greift jetzt auch auf der eingebetteten Dokumentation (das isolierte iframe fing die Scroll-Geste vorher ab; oben/unten anstossen blaettert nun zur Nachbar-Seite).
- **Schaltbare Maschinen: Steckdose bleibt nach dem Lauf an, und die Buttons sind eine Steckdosen-Fernbedienung (Stufe 2+4)** — nach einem von SEA gesteuerten Durchlauf schaltet SEA die Steckdose wieder ein (Maschine ist direkt einsatzbereit). Die Buttons auf „Heute" und unter Vorgaben schalten jetzt die Steckdose: **an SEA übergeben** = aus (SEA übernimmt + verschiebt bis zur Fertig-Zeit), **sofort starten** = ein, **Auftrag zurücknehmen** = ein + Auftrag verwerfen. Ein laufender, direkt gestarteter Lauf kann so per Knopf noch an SEA übergeben werden.
- **Schaltbare Maschinen: Steuerung kehrt den Auslöser um (Stufe 1)** — SEA verschiebt einen an der Maschine gestarteten Lauf nicht mehr automatisch. Ein Direktstart (Steckdose ein) wird nur noch **beobachtet** (läuft/Ende erkannt, für die Fertig-Meldung), SEA fasst den Schalter nicht an. Wer verschieben will, **schaltet die Steckdose aus** — SEA übernimmt dann die Maschine als gemanagten Auftrag (Frist verpflichtend) und disponiert im Fenster. Die Fertig-Meldung kommt in **jedem** Fall; Verschiebungs-/Start-Ansagen nur bei gemanagten Läufen. (Weitere Stufen: Einschalten im Fenster + Buttons als Steckdosen-Fernbedienung.)
...
- **Neue Karte „SEA-Einsparung aus Entscheidungen"** — was SEAs Eingriffe im Moment ihrer Ausführung eingespart haben, je Art (Speicher-Eigenverbrauch, Überschuss→Wärme, Lastverschiebung), mit dem echten Preis/PV des Augenblicks aus den aufgezeichneten Ist-Werten verbucht. Kein simulierter Batterie-Walk → stabil gegen Aufzeichnungslücken (die den rekonstruierten Baseline-Vergleich 30-fach schwanken ließen). Speicheranteil wirkt rückwirkend, Wärme/Verschiebung ab jetzt.
- **„Ohne Steuerung" schreibt jetzt auch eine Lastverschiebungs-Gutschrift gut** — Überschuss, den SEA in eine verschobene Last lenkt (Waschmaschine ins PV-Fenster, EV in die Sonnenstunden), wird der Baseline als entgangene Einspeisung plus Netzbezug zur natürlichen Zeit angelastet (Preis − Einspeisung). Zusammen mit der Senken-Gutschrift wird so SEAs Optimierung sichtbar, die ein reiner Batterie-Vergleich strukturell verschluckt (neu je Snapshot aufgezeichnet, wirkt ab jetzt).
- **JS/CSS werden mit Versions-Query ausgeliefert (Cache-Busting)** — die Asset-URLs in index.html tragen jetzt `?v=<Version>`, sodass Cache-Schichten, die `no-cache` ignorieren (HA-Ingress, mobiler Webview), nach einem Update kein altes Bundle mehr ausliefern können. Löst, dass ein Handy die neue Oberfläche zeigt und ein anderes die alte.
- **Jede Batterie-Karte auf der Status-Seite zeigt jetzt ihren geschätzten SoH / die nutzbare Kapazität** — aus den aufgezeichneten Lade-/Entlade-Energien und dem SoC-Hub je Batterie (neue Per-Batterie-Aufzeichnung), Median über saubere Segmente, im Verhältnis zur eingestellten Nennkapazität.
- **„Ohne Steuerung" schreibt SEA jetzt eine Senken-Gutschrift gut** — Überschuss, den SEA bei vollem Speicher in Warmwasser (Heizstab/ELWA) statt in den Export lenkt, wird der Baseline als entgangene Einspeisung plus Wärmepumpen-Ersatzstrom (Preis/COP) angelastet. So wird dieser SEA-Nutzen sichtbar, den ein reiner Batterie-Vergleich verschluckt. Dafür wird die überschuss-gedeckte Senken-Leistung neu je Snapshot aufgezeichnet (wirkt nur für ab jetzt aufgezeichnete Zeiträume).
- **Die Vergleichsszenarien rechnen jetzt mit realistischem Wirkungsgrad und denselben Batteriegrenzen wie die echte Anlage** — Umrichter 90 % je Richtung (81 % Roundtrip statt vorher zu optimistischen 90 %) und der konfigurierte Notreserve-SoC als Entlade-Boden gelten jetzt auch für „Ohne Steuerung" und die Tarifmodelle. Vorher war der dumme Speicher systematisch zu gut (verlustfrei, bis 0 % entladbar), was den Vorteil der Steuerung kleinrechnete.
- **Tag- und Wochen-Werte der Einsparung/Tarifvergleiche stimmen jetzt überein** — die Gegenrechnungs-Batterie („Ohne Steuerung" und die Tarifmodelle) startet am Fensteranfang mit dem **tatsächlich aufgezeichneten Ladestand** statt leer. Vorher startete jedes Fenster mit leerem Akku, sodass Einzeltage den dummen Speicher jeden Morgen künstlich bestraften — die Summe der 7 Tage widersprach dem Wochenwert. Jetzt beginnt jedes Fenster (Tag/Woche/Monat) beim realen Füllstand.
- **Die Analyse-Seite hat einen gemeinsamen Zeitraum-Selektor (wie Verläufe) und die Einsparung ist eine Baseline-Tabelle** — oben eine Zeitraum-Karte (Tag/Woche/Monat/Jahr = rollend rückwärts, „bis jetzt" = datumsgenau bis heute/diese Woche/…, ◀ ▶ blättert ganze Zeiträume, dazu die effektive Datumsspanne) — derselbe Code wie auf der Verläufe-Seite. „Einsparung durch Optimierung" zeigt jetzt **alle drei Baselines als Tabelle** (Kosten + Ersparnis vs. Ist, Info-Knopf je Zeile) statt eines Auswahlmenüs; Tarifvergleich und Investition teilen sich denselben Zeitraum. Im Tarifvergleich zeigt die Zeile deines **eigenen** Tarifs die realen Ist-Kosten (markiert), die anderen sind Vergleichsrechnungen.
- **Der Tarifvergleich passt jetzt zu den Ist-Kosten und zeigt immer alle Tarifmodelle** — statt einer Re-Simulation (die auf der reinen Grundlast rechnete und zu niedrige Kosten zeigte) wird jetzt der **tatsächlich aufgezeichnete Netzbezug** unter jedem Tarifmodell neu bewertet. Die Zeile deines **eigenen** Tarifs zeigt die realen Ist-Kosten (markiert), die übrigen Modelle sind Vergleichsrechnungen — und werden **immer** angeboten (auch ohne eigenen HT/NT- oder dynamischen Vertrag). HT/NT und dynamisch nutzen deinen Speicher zur Arbitrage (laden wenn günstig, entladen wenn teuer), dynamisch verschiebt zusätzlich verschiebbare Geräte ins günstigste Tagesfenster; ohne echte Börsen-Historie dient ein Tag/Nacht/Spitzen-Modell als Näherung. Modelliert mit deiner echten Batterieleistung.
- **Drei klar benannte Einsparungs-Baselines** — der Vergleich der Einsparung lässt sich jetzt gegen **Ohne Anlage** (weder PV noch Speicher → Gesamtwert der Anlage), **Ohne Speicher** (PV, kein Speicher → Wert von Speicher+Steuerung) oder neu **Ohne Steuerung** (PV + dein Speicher, aber dumm im Eigenverbrauch → isoliert den Wert der SEA-Steuerung allein) rechnen.
...
- **Kein 5-Stunden-Phantom mehr bei der Wärmepumpen-Anhebung im Ausblick** — lässt sich die Speichergröße der WP nicht aus der History schätzen (die WP-Leistung mischt Raumheizung und Warmwasser, das passt schlecht zusammen), bekam der simulierte Warmwasser-Speicher gar kein Modell und die Anhebung lief im Ausblick stundenlang durch (~20 kWh in den Tank). Jetzt fällt die Simulation auf einen Standard-Warmwasserspeicher (~200 L) zurück, sodass die Anhebung nach realistischer Aufheizzeit **sättigt** (Größenordnung 15 min) statt den ganzen Überschuss-Zeitraum zu belegen.
- **Der Planungshorizont ist auf 16 Stunden begrenzt** — darüber hinaus ist die Prognose zu unsicher; ein Fenster so weit voraus verschiebt sich ohnehin oder wird näher am Termin neu gesetzt. Ausblick, Planer, Zielprüfung und das Prognose-Diagramm schauen jetzt höchstens so weit.
- **Dynamische (Börsen-)Tarife verschieben jetzt auch in der Simulation** — bisher wurde ein dynamischer Tarif in Analyse/Ausblick nur nachbepreist, aber die simulierte Regelung reagierte nicht darauf (der Sim-Klon hatte kein Preissignal). Jetzt bekommt die Simulation die Preisreihe pro Schritt (aufgezeichnete bzw. Prognose-/Börsenpreise), sodass die echte Regelung im Zeitraffer in die günstigen Fenster verschiebt — Rückblick, Investitions-Analyse, Jahresübersicht und Ausblick rechnen dynamische Tarife damit verhaltensecht statt als Untergrenze.
- **Die Jahresübersicht rechnet feste Investitionen übers Jahr durch** — der Nacht-Job simuliert jeden Tag zusätzlich mit **Batterie +50 %/+100 %**, **PV +50 %/+100 %** und **beidem zusammen (+100 %/+100 %)** und legt sie im Cache ab; die Karte zeigt darunter die Ersparnis je Option im Zeitraum und aufs Jahr hochgerechnet (unter deinem Tarif bewertet, ohne Anschaffungskosten) — über ein ganzes Jahr, aus dem Cache statt neu gerechnet.
- **Die Jahresübersicht zeigt bei dynamischem Tarif die echten dynamischen Kosten** — der Tages-Cache speichert jetzt auch die tatsächlich aufgezeichneten Preise, sodass die Spalte „Dynamisch" die realen Kosten deines Börsentarifs summiert (statt zu fehlen), neben den Vergleichswerten für Fest und HT/NT.
- **Neue Analyseart „Jahresübersicht" — wie deine Anlage tatsächlich lief** — unter *Verhalten → Analysen* fasst sie bis zu einem Jahr zusammen (Stromkosten und Autarkie je Tarifmodell), **summiert aus nachts vorberechneten Tagessimulationen** statt den ganzen Zeitraum neu zu rechnen — dadurch auch über lange Zeiträume schnell. SEA simuliert jeden abgeschlossenen Tag einmalig nachts (03:00) unter den damals aktiven Einstellungen und legt das Ergebnis kompakt ab (überlebt die Rohdaten-Aufbewahrung); der erste Aufruf holt fehlende Tage nach. Dieselben Tage werden je Tarifmodell (Fest, HT/NT) nur neu bepreist — dynamische Tarife sind hier noch nicht enthalten.
- **Die Verlaufsseite zeigt jetzt die prognostizierten Ladestände und Speichertemperaturen** — neben den aufgezeichneten Kurven laufen gestrichelt die Vorausschau für Batterie- und Auto-Ladestand sowie für Warmwasser- und Wärmepumpen-Speicher weiter, sodass „reicht der Ladestand?" bzw. „wird das Wasser warm genug?" direkt an der Linie ablesbar ist (aus derselben Ausblick-Simulation, die auch den Plan erzeugt).
- **Auto sofort laden / Laden beenden** — unter *Verhalten → Vorgaben & Grenzen* gibt es beim Fahrzeug jetzt Knöpfe: „Sofort laden" lädt umgehend (nötigenfalls aus dem Netz) bis zum Mindest-Ladestand — oder, wenn der schon erreicht ist, bis zum Maximal-Ladestand — und stoppt dann; „Laden beenden" hält das Laden an; „Automatik" gibt die Kontrolle wieder an SEA. Der Zielwert wird beim Drücken festgehalten.
- **Die Wärmepumpe wird jetzt kostenbasiert vorausgeplant** — der Planer legt das Vorheiz-Fenster ins *günstigste* Fenster nach gestaffelten Grenzkosten (PV-Überschuss zum Einspeisewert, Rest zum Netzpreis) geteilt durch den COP, und heizt nur vor, wenn das echtes On-Demand-Heizen aus dem Netz unterbietet. (Pro kWh Wärme dominiert die WP dank COP ohnehin den Heizstab; der Heizstab bleibt die reaktive, feinregelnde Ergänzung für kleinen Überschuss.)
- **Die Wärmepumpe in der Prognose berücksichtigt jetzt den echten Speicherzustand** — der Ausblick-Zeitraffer modelliert den Wärmespeicher der WP mit seiner tatsächlichen Starttemperatur (aus dem Verlauf) und heizt nur noch, solange Temperatur-Reserve zum angehobenen Ziel besteht. Ist der Speicher schon warm, erscheint keine „Wärme" mehr in der Spur, die es real nicht gibt.
- **Assistierter Modus: Plan übernehmen** — statt bei jedem vorausschauenden Abweichen einzeln zuzustimmen, übernimmt man den Tagesplan einmal; er wird dann eingefroren ausgeführt. SEA plant weiter alle 15 min, meldet einen günstigeren Plan (einmalige Benachrichtigung) und bietet ihn zur Übernahme an; im Ausblick gibt es dafür „Plan übernehmen" / „Neuen Plan übernehmen" / „Freigeben". Im Automatik-Modus folgt SEA dem Plan still ohne Nachfragen.
- **„Eigenverbrauch" ist als Ziel entfallen** — es fällt bei normaler (niedriger) Einspeisevergütung ohnehin mit „Kosten" zusammen; es bleiben die zwei greifbaren Ziele **Kosten** und **Autarkie**.
- **Der Plan führt jetzt Batterie, verschiebbare Geräte und das Auto (nur im Automatik-Modus)** — statt myopisch aus dem Momentanwert zu entscheiden, gibt der vorausschauende Plan der bestehenden Regelung ihre Stellgröße vor: die Batterie ihren Modus (Netzladen / gezieltes Entladen / Überschuss folgen), ein verschiebbares Gerät sein geplantes Lauffenster, das Auto sein günstigstes Ladefenster (die Wallbox lädt dann darin, nötigenfalls aus dem Netz, bis zum Mindest-Ladestand vor der Abfahrt). Die reaktive Regelung selbst (Modulation, Bank-Aufteilung, Reserve, Schwellen, Schreibtakt, SoC-unbekannt-Schutz) bleibt unverändert; in „assistiert"/„manuell" entscheidet weiter die reaktive Regelung und der Plan ist nur ein Vorschlag.
- **Prognose und Plan rechnen jetzt mit allen steuerbaren Batterien** — bisher sah der Planer nur die erste; jetzt bildet er einen virtuellen Puffer aus allen von SEA gesteuerten Bänken (summierte Kapazität/Leistung, kapazitätsgewichteter Ladestand), und die Prognose-Simulation führt jede Bank einzeln (eine volle Bank lädt nicht weiter, während eine andere lädt). Eine Batterie im Automatikmodus (eigener „Automatik"-Schalter oder ganz ohne Stellgröße) wird von SEA nicht angesteuert und in der Prognose als selbstverwaltende Hintergrundlast geführt (deckt Überschuss/Defizit selbst), statt als untätig zu gelten; die Anhebe-Reserve der Solltemperatur berücksichtigt jetzt ebenfalls alle steuerbaren Bänke gemeinsam.
- **Ausgeklappte Kind-Knoten im Flussdiagramm überlappen nicht mehr** — der Zeilenabstand richtet sich jetzt nach der zweiten Beschriftungszeile (dem Ladestand), sodass die „SoC …"-Zeile einer Batterie-Bank bzw. eines Autos nicht mehr in den Kreis des nächsten Kindes läuft (sichtbar bei zwei Speichern oder zwei Fahrzeugen).
- **Mehrere PV-Anlagen lassen sich im Flussdiagramm aufklappen** — sind mehrere PV-Anlagen konfiguriert, wird der „PV"-Knoten mit „+" zum Elternknoten: jede Anlage erscheint als Kind mit eigener Leistung, am Desktop horizontal aufgefächert (letzte genau unter dem Knoten, weitere nach links) — mit nur einer Beschriftungszeile, da eine PV-Anlage keinen Ladestand hat. Der „Haus"-Knoten rückt dabei automatisch so weit nach unten, dass die Anlagenreihe darüber Platz hat.
- **Mehrere Batterie-Bänke fächern jetzt nebeneinander auf** — am Desktop hängt die letzte Bank genau unter dem „Batterie"-Knoten, die weiteren stufen nach links; so verlässt jede Bank-Flusslinie den Knoten in eigenem Winkel und ist unterscheidbar (statt alle senkrecht übereinander). Zudem beginnt die erste Bank tiefer, damit die SoC-Zeile des Batterie-Knotens selbst frei steht; am Handy bleibt der senkrechte Stapel.
- **Mehrere Batterien werden einzeln angezeigt** — sind mehrere Speicher konfiguriert, hängt im Flussdiagramm jede Bank als aufklappbares Kind unter einem gemeinsamen Batterie-Knoten, dessen SoC jetzt **kapazitätsgewichtet** ist (eine kleine Bank bei 20 % neben einer großen bei 80 % ergibt nicht mehr „50 %"); das Sankey-Diagramm zeigt eigene Lade-/Entlade-Flüsse je Bank und die „Aktuell"-Karte den SoC jeder Batterie getrennt.

## 0.10.10

- **Die „Jetzt"-Zeile im Tagesplan zeigt die tatsächlich laufende Strategie** (dieselbe wie in der „Aktuell"-Karte) statt der eventuell veralteten Prognose-Öffnung — kein „Batterie deckt Netzbezug" mehr, während gerade massiv PV-Überschuss eingespeist wird.
